...
public abstract class AbstractQueryCreator<T> implements QueryCreator<T> {
    ...
    private static final boolean STANDARD_DELETED_OPTION = false;

    protected List<Predicate> addCheckDeleting(MultiValueMap<String, String> fields, CriteriaBuilder criteriaBuilder, Root<T> root) {
        List<Predicate> restrictions = new ArrayList<>();
        String deleted = getSingleMultiValueMapParameter(fields, DELETED);
        if (deleted != null) {
            restrictions.add(criteriaBuilder.equal(root.get(DELETED), Boolean.parseBoolean(deleted)));
        } else {
            restrictions.add(criteriaBuilder.equal(root.get(DELETED), STANDARD_DELETED_OPTION));
        }

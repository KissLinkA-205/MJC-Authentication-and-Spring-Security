...
public class GiftCertificateServiceImpl extends AbstractService<GiftCertificate> implements GiftCertificateService {
    ...
    private static final boolean NOT_DELETED_FLAG = false;

    @Autowired
    public GiftCertificateServiceImpl(JpaRepository<GiftCertificate, Long> dao, GiftCertificateDao giftCertificateDao,
                                      DateHandler dateHandler, Renovator<GiftCertificate> renovator, Renovator<Tag> tagRenovator) {
        super(dao);
    ...
    public GiftCertificate insert(GiftCertificate giftCertificate) {
        ...
        giftCertificate.setDeleted(NOT_DELETED_FLAG);

        return dao.save(giftCertificate);
    }
    ...
    public void removeById(long id) {
        ...
        giftCertificateDao.setDeletedById(DELETED_FLAG, id);
    }
    ...
    public GiftCertificate update(long id, GiftCertificate giftCertificate) {
        ...
        return giftCertificateDao.save(newGiftCertificate);
    }
    ...
    public List<GiftCertificate> doFilter(MultiValueMap<String, String> requestParams, int page, int size) {
        ...
        String deletedFlag = getSingleRequestParameter(requestParams, DELETED);
        if(deletedFlag != null) {
            IdentifiableValidator.validateDeletedFlag(deletedFlag, exceptionResult);
        }
        if (!exceptionResult.getExceptionMessages().isEmpty()) {
            throw new IncorrectParameterException(exceptionResult);
        }
